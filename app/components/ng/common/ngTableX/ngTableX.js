(function(angular, factory) {
     ...
     */
    var app = angular.module('ngTableX', []);
    /**
     * ngTable: Table + Angular JS
     *
     * @author Vitalii Savchuk <esvit666@gmail.com>
        ...
        var ngTableParams = function (baseParameters, baseSettings) {
            ...
            this.reload = function () {
                ...
                $defer.promise.then(function (data) {
                    ...
                    settings.$scope.$emit('ngTableAfterReloadData',settings.$scope.$data);
                });
            };
...
//            showDefaultPagination:true,
    ...
    var ngTableController = ['$scope', 'ngTableParams', '$timeout', function ($scope, ngTableParams, $timeout) {
        ...
        $scope.sortBy = function (column, event) {
            ...
            var defaultSort = $scope.params.settings().defaultSort || 'asc'; //fix for sorting issue
...
//                                scope.$emit('ngTableParamsChanged', {tableParams: attrs.ngTable, scope: scope});
//                            }
//                            scope.paramsModel = $parse(attrs.ngTable);
//                            scope.params = params;
//                        }), true);
//
//                        scope.$watch('params.settings().showDefaultPagination', (function (newValue,oldValue) {
//                            if (angular.isUndefined(newValue)||newValue==oldValue) {
//                                return;
//                            }
//                            if (newValue == false ){
//                                // if user do not use the default ,remove the new added pagination
//                                element.parent().find('[ng-table-pagination]:first').remove();
//
//                            }else {
//                                // from false to true ,means user want to add default  pagination at last
...
//                                noMatch:(attrs.templateNoMatch ? attrs.templateNoMatch : 'ng-table/noMatch.html')
//                            };
...
//                                'template-url': 'templates.pagination'
//                            });
//                            var noMatchTemplate = angular.element(document.createElement('tr')).attr({
//                                'ng-if':'$data.length==0',
//                                'ng-include':'templates.noMatch'
//                            });
//                            element.find('thead').remove();
//
//                            element.addClass('ng-table')
//                                .prepend(headerTemplate).after(paginationTemplate);
//                            element.append(noMatchTemplate);
//
//                            $compile(headerTemplate)(scope);
//                            $compile(paginationTemplate)(scope);
//                            $compile(noMatchTemplate)(scope);
//                            scope.$emit('ngTableAfterInited');
//                        }
//
//                    };
//                }
    ...
    angular.module('ngTableX').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('ng-table/noMatch.html','<td valign="top" colspan="{{$columns.length}}" ><i class="fa fa-warning text-warning"></i> No matching records found</td>')
    }]);
    return app;
}));