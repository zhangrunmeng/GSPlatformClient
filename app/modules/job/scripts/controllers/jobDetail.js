...
/**
 * @ngdoc function
 * @name jobManagement.controller:JobdetailctrlCtrl
 * @description
 * # JobdetailctrlCtrl
 * Controller of the jobManagement
 */
define(['angular'], function(angular){
    return function ($scope,$element,Restangular,ngTableParams,Utility) {
        ...
        $scope.onResize = function(evt, scala){
            $scope.historyTableHeight = (scala.height - 135) + "px";
            $scope.$apply();
            $scope.scmSettingHeight = (scala.height - 340) + 'px'
            $element.find("div[class='tab-content']").css('height', scala.height - 48 - 20 + 'px');
        }

        $scope.$on('contentResize', $scope.onResize);
